...
//! writes both to disk under `<daemon_dir>/deployments/<slug>/v<n>/`,
...
fn deploy_writes_filesystem_and_db_row() {
    ...
    let dv = daemon
        .deploy(slug, RECIPE.to_string(), catalog.clone())
        .expect("deploy");
    assert_eq!(dv.version, 1);
    assert_eq!(dv.slug, slug);

    let v1_dir = ws_root.join(".forage").join("deployments").join(slug).join("v1");
    ...
    let current = daemon.current_deployed(slug).unwrap();
...
fn deploy_rejects_broken_source() {
    ...
    assert!(daemon.current_deployed(slug).unwrap().is_none());
}
...
fn deploy_rejects_invalid_recipe() {
    ...
    let ws_root = tmp.path().to_path_buf();
    let slug = "validates";
    init_workspace(&ws_root, slug, RECIPE_REFERENCES_UNDECLARED_TYPE);
    let daemon = Daemon::open(ws_root.clone()).unwrap();

    let catalog = catalog_for(RECIPE_REFERENCES_UNDECLARED_TYPE, &ws_root);
    let err = daemon
        .deploy(slug, RECIPE_REFERENCES_UNDECLARED_TYPE.to_string(), catalog)
        .expect_err("validation failure rejects the deploy");
    assert!(matches!(err, DeployError::Validate(_)), "got {err:?}");

    let v1_dir = ws_root.join(".forage").join("deployments").join(slug).join("v1");
    assert!(!v1_dir.exists(), "no version directory must be written on validation failure");
    assert!(daemon.current_deployed(slug).unwrap().is_none());
}
...
fn deploy_bumps_version() {
    ...
    let v1 = daemon.deploy(slug, RECIPE.to_string(), catalog.clone()).unwrap();
    let v2 = daemon.deploy(slug, RECIPE.to_string(), catalog).unwrap();
    assert_eq!(v1.version, 1);
    assert_eq!(v2.version, 2);

    let deployments_root = ws_root.join(".forage").join("deployments").join(slug);
...
fn deploy_updates_run_pointer_when_run_exists() {
    ...
    let catalog = catalog_for(RECIPE, &ws_root);
    daemon.deploy(slug, RECIPE.to_string(), catalog).unwrap();
