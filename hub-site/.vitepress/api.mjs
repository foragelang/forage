...
export async function fetchRecipeList(base = HUB_API) {
    try {
        const r = await fetch(`${base}/v1/recipes?limit=100`)
        if (!r.ok) {
            console.warn(`[hub-site] /v1/recipes returned ${r.status}; skipping`)
            return []
        }
        const data = await r.json()
        return Array.isArray(data.items) ? data.items : []
    } catch (err) {
        console.warn(`[hub-site] fetch ${base}/v1/recipes failed:`, err?.message ?? err)
        return []
    }
}
...
export async function fetchRecipeDetail(slug, base = HUB_API) {
    try {
        const r = await fetch(`${base}/v1/recipes/${encodeURIComponent(slug)}`)
        if (!r.ok) return null
        return await r.json()
    } catch (err) {
        console.warn(`[hub-site] fetch ${base}/v1/recipes/${slug} failed:`, err?.message ?? err)
        return null
    }
}
