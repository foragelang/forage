/// Pins the gutter-click → breakpoint flow: clicking a pause-able
/// statement's gutter line (step / emit / for) toggles a breakpoint,
/// and the binding stays live across the async `recipeOutline`
/// arrival — i.e. the click handler reads the current pause-point
/// map rather than a snapshot captured at mount time.
...
const fakeEditor = {
    ...
    onMouseMove(_h: MouseHandler) {
        return { dispose: () => {} };
    },
    onMouseLeave(_h: () => void) {
        return { dispose: () => {} };
    },
...
describe("EditorPane gutter click", () => {
    ...
    beforeEach(() => {
        ...
        // Use a non-zero gcTime so `setQueryData` seeds survive
        // long enough for `recipeNameForPath` to read them at
        // toggle time.
        qc = new QueryClient({
            defaultOptions: { queries: { retry: false } },
        });
        installStudioService(service, qc);
        // Seed the recipe status cache so `recipeNameForPath` can
        // resolve `shop/recipe.forage` → `"shop"` synchronously.
        const seed: RecipeStatus[] = [
            {
                name: "shop",
                draft: { kind: "valid", path: "shop/recipe.forage" },
                deployed: { kind: "none" },
            },
        ];
        qc.setQueryData(recipeStatusesKey(), seed);
    ...
    test("click on a step line toggles breakpoint after outline loads", async () => {
        // Mirror the cache seed so the polling refetch doesn't blow
        // away the recipe-status data we need for the path → name
        // join.
        service.setHandler("listRecipeStatuses", async () => [
            {
                name: "shop",
                draft: { kind: "valid", path: "shop/recipe.forage" },
                deployed: { kind: "none" },
        ...
        // re-render to flush the new pause-point map.
        await waitFor(
            () => {
        ...
        // Give the .then() handler a microtask to commit the new pause
        // points and let React commit the resulting re-render.
        await act(async () => {
            await new Promise((r) => setTimeout(r, 50));
        });

        // The pause point's 0-based start_line=4 maps to Monaco's
        // 1-based line 5.
        ...
        await waitFor(() => {
            ...
            // Wire-side: recipe header name + 0-based source line.
            expect(bpCalls[0]!.args).toEqual(["shop", [4]]);
        });
        // Store should reflect the toggled breakpoint too.
        expect([...useStudio.getState().breakpoints]).toEqual([4]);
    });
});
