...
The Toolkit is a SwiftUI macOS app for authoring Forage recipes
...
Everything the CLI can do, the Toolkit can do. The difference is
...
cd forage/Toolkit
xcodegen
xcodebuild -project Toolkit.xcodeproj -scheme Toolkit -configuration Debug build
open ~/Library/Developer/Xcode/DerivedData/Toolkit-*/Build/Products/Debug/Toolkit.app
```

The app runs from the build directory; copy `Toolkit.app` to
`/Applications` if you want it permanent. The Toolkit lives in the
`Toolkit/` subdirectory of the Forage repo and depends on the local
Forage Swift package — no third-party dependencies.
...
When you create a new recipe from the sidebar, the Toolkit scaffolds
this layout for you with a minimal template.

## First recipe

1. Launch the Toolkit. The sidebar lists every slug under
...
3. Edit the slug folder name on disk (or rename the file path) if you
   want a meaningful name. The Toolkit picks up the change on the next
   sidebar refresh.
...
When the run finishes, the Toolkit jumps to the **Snapshot** tab. Each
...
3. Click **Preview payload** to see the JSON body the Toolkit would
   POST to `api.foragelang.com`.
4. Click **Publish**.

::: warning
M3 ships the publish flow in stub mode. The Toolkit prints "would POST
...
- **API key** — pasted, then saved to the user's login Keychain.
  Stored under the `com.foragelang.Toolkit` service. Delete with the
  matching button.
...
- **Hub publish is stubbed.** M4 wires the real POST; for now Publish
  shows what it would send.
- **Import from hub is disabled.** Same — M4 brings the live `GET`
  flow.
- **Tabs aren't editable from inside the editor.** Renaming a slug is
  ...
  the sidebar).
- **No multi-recipe windows.** Open one slug at a time per Toolkit
  window. macOS lets you open a second Toolkit window from the File
  menu if you need it.
